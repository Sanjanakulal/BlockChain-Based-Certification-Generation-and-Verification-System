...
const showToast = (message, type = "success", autoCloseMs = 5000) => {
  // Create the custom toast and capture its id
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ width: "100%", animation: `shrink ${autoCloseMs}ms linear forwards` }}
        />
      </div>
  ...
  // Schedule dismissal using the returned id (outside the render callback)
  if (id) {
    setTimeout(() => {
      try {
        toast.dismiss(id);
    ...
    // fallback: try to dismiss anything if id wasn't returned
    setTimeout(() => toast.dismiss(), autoCloseMs);
  }
